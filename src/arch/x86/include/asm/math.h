...
#define div_asm(res, div, val)	\
	asm volatile("divl %2" : "=a"(res) : "rm"(div), "0"(res))

#define div_rem_asm(res, div, val, rem)	\

static inline uint64_t arch_div(uint64_t val, uint32_t div, uint32_t *rem)
{
	uint32_t upp, low;
	uint64_t result = 0;
